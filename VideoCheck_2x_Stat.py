...
# 新しい　nkawa1932 の動画は、　3932x2312 になってる(crop/mask が違うため)(もとは　3990x2312)
# 右側を触っただけだから、左は関係ないと信じたいw
# stich の仕組みが違うため、位置の計算の変更が必要
# さらに、 x2 (つまり 18000フレームじゃなくて 9000フレーム化している)

# 元データが、3932x2312 ピクセルの動画で、縮小動画は 1280x752ピクセル
# 元のトラッキングの位置から、 3885.36, 812.703 シフトさせると、元の動画の位置になる。これを 1280x752 に変換
# 1280/3990 = 0.321303
# 1280/3932 = 0.325534
...
BASE_X = 3885.356
BASE_Y = 812.703
SCALE = 1280/3932  #0.355534

TGT_WIDTH = int(3932*SCALE)
TGT_HEIGHT = int(2312*SCALE)

MAX_VIDEO_FRAME = 8999
...
cscale = 1.2
cw = int(TGT_WIDTH*cscale)
ch = int(TGT_HEIGHT*cscale)
...
#        self.kakudai.delete("all")
        ...
        self.cbox = tk.BooleanVar(value=True)
        ...
        self.check_wbox = tk.Checkbutton(self.check_frame, text="Show Box", width=10,variable = self.cbox)
        self.check_wbox.pack(side=tk.LEFT,padx=10)
...
# IDを登録する仕組み！
        ...
        self.slider = tk.Scale(self.slider_frame, 
                               ...
                               command=self.scroll,
                               from_=0, to=MAX_VIDEO_FRAME,length=1400, orient=HORIZONTAL)
        self.ssd = 0
        self.sed = MAX_VIDEO_FRAME
        self.scl = int(90/7)
...
#                boximage = self.base_image[yy-10:yy+int(height)+10, xx-10:xx+int(width)+10]
                boximage = self.orgimg[yy-30:yy+int(height)+30, xx-30:xx+int(width)+30]
                ...
                colorconv = cv2.cvtColor(boximage, cv2.COLOR_BGR2RGB)
                pil3_image = Image.fromarray(colorconv)
...
#        print("Click",x,y)
        ...
        if self.workers is not None:
            fnum = [self.frameVar.get()]  #ああ、frameID でチェックが必要だった
            for frameData in self.workers:
...
#        print("Current Frame",frame,self.cap.get(cv2.CAP_PROP_POS_FRAMES))
        if app.frame != frame:
            self.cap.set(cv2.CAP_PROP_POS_FRAMES,frame)
            print("Set Frame",frame,self.cap.get(cv2.CAP_PROP_POS_FRAMES))
            app.frame = frame
        ...
        tf, self.orgimg = self.cap.read()
        # 画像を半分のサイズにしたい
        if not tf:
#            print("Capture failed for",self.cap)
            return
        img = cv2.resize(self.orgimg,dsize=(TGT_WIDTH,TGT_HEIGHT))
...
# ボックス状態の描画
        ...
        if self.cbox.get():
            for bnum in range(1, len(ptokai_box_info)+1):
                cx, cy = ptokai_box_info[bnum][0] # box の中心位置を取得
                cx = int((cx- BASE_X)*SCALE)
                cy = int((cy- BASE_Y)*SCALE)
                wid = int(50*SCALE)
                base_image = cv2.rectangle(base_image,(cx-wid,cy-wid),(cx+wid,cy+wid),(200,200,200), thickness=1)
        
            if self.boxes is not None:
                fdata = self.boxes[frame]
                bnum = 1
                for pred in fdata:
#                    print("Box", pred)
                    cx, cy = ptokai_box_info[bnum][0] # box の中心位置を取得
                    ...
                    cy = int((cy- BASE_Y)*SCALE)
                    wid = int(30*SCALE)
                    base_image = cv2.rectangle(base_image,(cx-wid,cy-wid),(cx+wid,cy+wid),bcolors[pred], thickness=1)
                    bnum +=1
...
def load_track_file(file):
    with open(file, 'rb') as f:
        data = pickle.load(f)
        print("Loaded", len(data['boxes']))
        return data['workers'], data['pallets'],data['boxes']
...
if __name__ == "__main__":
    ...
    app.frame =0
#    (app.workers , app.pallets, app.boxes ) = load_track_file('track_2x_20241104.pkl')
#    (app.workers , app.pallets, app.boxes ) = load_track_file('track_2x_20241105_bibs.pkl')
    (app.workers , app.pallets, app.boxes ) = load_track_file('track_2x_20241106_bibs_ymori_true.pkl')

    app.track_colors = {track['track_id']: generate_random_color() for frame in app.workers for track in frame['tracks']}
    app.ptrack_colors = {track['track_id']: generate_random_color() for frame in app.pallets for track in frame['tracks']}
    app.next()
